...
mergeSort:
	xor r10, r10		;zero it out to use, int mid
	push rsi		;need OG start index later
	push rdx		;need OG end index later
	; Implement mergeSort here
	cmp rsi, rdx		;if(start < end)
	jl start		;
	jge invalid_call	;else
start:
	add r10, rsi		;add start
	add r10, rdx		;add end
	shr r10, 1		;int mid = (start + end) / 2

	push r10		;need OG mid later
	mov rdx, r10		;3rd parameter = mid
	call mergeSort		;mergeSort(A, start, mid)
	
	pop r10			;recover OG mid
	pop rdx			;recover third parameter
	pop rsi			;recover second parameter
	
	push rsi		;store rsi
	push rdx		;store rdx
	push r10		;store OG mid again for later
	
	inc r10			;mid++
	mov rsi, r10		;start = mid+1
	...
	mov rcx, rdx		;4th parameter set to end
	mov rdx, r10		;3rd parameter set to mid
