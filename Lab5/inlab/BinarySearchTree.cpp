...
BinaryNode* BinarySearchTree :: insert(BinaryNode* n, const string& x){
  ...
  if(x < n->value){
    BinaryNode* left = insert(n->left, x);
    n->left = left;
  }
  if(x > n->value){
    BinaryNode* right = insert(n->right, x);
    n -> right = right;
  }
  return n;
}
...
string BinarySearchTree::pathTo(const string& x) const {
    // YOUR IMPLEMENTATION GOES HERE
  string output = pathTo(root, x);
  if(output.substr(output.length()-x.length()) != x){
    return "";
  }
  return output;
}
...
string BinarySearchTree::pathTo(BinaryNode* n, const string& x) const{
  string output = "";
  if(n == nullptr){//checks if the root is null or if x exists or not
    return "";
  }
  if(x == n->value){//if the root is what we want, just return it
    output = n->value;
    return output;
  }
  if(x < n->value){//if the x is less than root, we travel down the left subtree
    output = n->value + " ";
    output += pathTo(n->left, x);
  }
  if(x > n->value){//if the x is less than root, we travel down the right subtree
    output = n->value + " ";
    output += pathTo(n->right, x);
  }
  return output;
}
...
string BinarySearchTree::min(BinaryNode* node) {
